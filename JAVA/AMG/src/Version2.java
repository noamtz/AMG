...
public class Version2 {
	...
	public static double[] apply_operator(double[] v){
		...
		double h = Math.pow((v.length+1), 2);
	...
	public static double[] restrict(double[] v){
		...
		for(int i=0; i<restricted.length; i++){
			...
			restricted[i] = 0.25 * (2*v[2*i] + before + after);
		}
		return restricted;
	}
	...
	public static double[] interpolate(double[] v){
		...
		for(int i=0; i<v.length; i++){
			double after = (i==v.length-1 || i==0 ) ? 0 : v[i+1];
			interpolated[2*i + 1] = v[i]; 
			interpolated[2*i] = 0.5*(v[i] + after);
		}
		return interpolated;
	}
	...
	public static int gridSize(int level){
		return (int)Math.pow(2, log2(N)-level)-1;
	}
